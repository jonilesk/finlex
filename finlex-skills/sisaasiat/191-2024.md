...
rakennuksiin kiinteästi asennettavien paloilmoittimien ja automaattisten sammutuslaitteistojen vaatimuksista, asennus-, huolto- ja tarkastustöistä, tarkastusvelvoitteesta ja tarkastuksen määrävälistä;

käsisammuttimien tarkastus- ja huoltotöistä, tarkastus- ja huoltovelvoitteesta ja tarkastuksen ja huollon määräväleistä.
...
sammutuspeitteet ja muut sellaiset käsin käytettävät, kannettavat alkusammutustuotteet, jotka on tarkoitettu tulipalon sammuttamiseen palon alkuvaiheessa;

palovaroittimet ja häkävaroittimet;

rakennukseen asennettavan paloilmoittimen osaksi tarkoitetut laitteet;

rakennukseen asennettavan automaattisen sammutuslaitteiston osaksi tarkoitetut laitteet;

rakennukseen asennettavaksi tarkoitetut sammutuslaitteet;

rakennukseen asennettavaksi tarkoitetut savunpoistoluukut, savunpoistopuhaltimet ja savusulut;

poistumisopasteet ja -valaisimet sekä muut poistumisreittien merkitsemiseen ja valaisemiseen tarkoitetut tuotteet;

panospolttoiset tehdasvalmisteiset tulisijat, jotka on tarkoitettu kiinteiden aineiden polttamiseen, rakennuksen savuhormiin liitettäviksi ja tuottamaan lämpöä tilaan, johon ne on asennettu;

väestönsuojien rakentamiseen ja varustamiseen tarkoitetut tuotteet.
...
paloilmoittimella laitteistoa, jonka tarkoituksena on havaita alkava tulipalo ja ilmoittaa siitä automaattisesti;

automaattisella sammutuslaitteistolla laitteistoa, jonka tarkoituksena on havaita tulipalo ja sammuttaa se tai pitää se hallinnassa;

asennusliikkeellä toiminnanharjoittajaa, joka tekee paloilmoittimien tai automaattisten sammutuslaitteistojen asennus- ja huoltotöitä;

tarkastuslaitoksella Turvallisuus- ja kemikaaliviraston hyväksymää organisaatiota, jolla on oikeus tehdä tässä laissa säädettyjä paloilmoittimien tai automaattisten sammutuslaitteistojen tarkastuksia;

käsisammuttimella kannettavaa ja käsin käytettävää sammutinta, jonka paino toimintakuntoisena on korkeintaan 20 kilogrammaa;

käsisammutinliikkeellä toiminnanharjoittajaa, joka tarkastaa tai huoltaa käsisammuttimia;

pelastusviranomaisella pelastuslain 26 §:n 2 momentissa tarkoitettua hyvinvointialueen pelastusviranomaista.
...
asennus- ja huoltotöissä noudatetaan tätä lakia;

laitteisto on tehdyn asennus- tai huoltotyön osalta lain vaatimusten mukainen;

asennus- ja huoltotöitä tekevillä henkilöillä on tarvittava osaaminen ja heidät on perehdytetty tehtäviinsä.
...
todistus suoritetusta 12 §:n mukaisesta vastuuhenkilön kokeesta;

yhteensä vähintään kolmen vuoden pituinen ja vastuuhenkilön tehtäviin nähden riittävän laaja-alainen työkokemus paloilmoittimien tai automaattisten sammutuslaitteistojen suunnittelu-, asennus- tai huoltotöistä.
...
paloilmoittimien asennus- ja huoltotyöt;

sprinklerilaitteistojen asennus- ja huoltotyöt;

vesisumulaitteistojen asennus- ja huoltotyöt;

kaasusammutuslaitteistojen asennus- ja huoltotyöt;

aerosolisammutuslaitteistojen asennus- ja huoltotyöt;

vaahtosammutuslaitteistojen asennus- ja huoltotyöt;

muu kuin 1–6 kohdassa tarkoitettu yksilöitävissä oleva ala.
...
laitteisto täyttää toteutukselle säädetyt vaatimukset;

laitteistosta on laadittu asennustodistus ja laitteisto on sen mukainen;

asennusliike on tehnyt laitteiston toimintatestaukset ja tarkastukset;

laitteisto on tarkoitukseen sopiva ja toimintakuntoinen;

laitteisto antaa ja välittää tarvittavat hälytykset ja ilmoitukset.
...
laitteiston hälytykset ja ilmoitukset ovat toimintakunnossa;

laitteiston muutos- ja korjaustyöt on teetetty asennusliikkeellä ja niistä on asianmukaiset asiakirjat;

laitteistolla on ajantasainen huolto- ja kunnossapito-ohjelma;

hätäkeskukseen kytketyn laitteiston yhteyshenkilön tiedot ovat ajan tasalla;

laitteiston kunnossapito on riittävää toimintakunnon ylläpitämiseksi.
...
tehdyn tarkastuksen ja seuraavan tarkastuksen ajankohta;

tarkastuksen kulku ja tehdyt tarkastustoimenpiteet;

selvitys laitteiston säännösten ja vaatimusten mukaisuudesta;

havaitut laitteistoon ja sen toimintakuntoon liittyvät puutteet.
...
laitos on Suomessa rekisteröity oikeushenkilö tai sen osa;

laitos on tarkastustehtäviä koskevassa toiminnassaan riippumaton ja puolueeton;

laitoksella on käytössään riittävästi ammattitaitoista henkilöstöä, jonka toiminnan riippumattomuus ja puolueettomuus tarkastustehtävissä on varmistettu;

laitoksella on käytössään toiminnan edellyttämät laitteet, välineet ja järjestelmät;

laitoksella on puolueettomasti arvioitu ja valvottu järjestelmä, jolla toiminnan laatu varmistetaan, sekä asianmukaiset ohjeet toimintaa ja sen seurantaa varten;
...
laitoksella on toiminnan laatu ja laajuus huomioon ottaen riittävä vastuuvakuutus toiminnasta aiheutuvien vahinkojen korvaamiseksi.
...
tarkastus- ja huoltotyössä noudatetaan tätä lakia;

tarkastus- ja huoltotöitä tekevillä on tarvittava osaaminen ja heidät on perehdytetty tehtäviinsä.
...
todistus suoritetusta 33 §:n mukaisesta vastuuhenkilön kokeesta;

yhteensä vähintään vuoden pituinen käsisammuttimien tarkastus- ja huoltotöihin perehdyttävä työkokemus.
...
osoitettava tuotteen vaatimustenmukaisuus;

koottava ja pidettävä saatavilla vaatimustenmukaisuuden osoittamista ja valvontaa varten tekniset asiakirjat;

varustettava tuote tarpeellisilla merkinnöillä ja tiedoilla;

laadittava tuotteen mukana toimitettavaksi käyttö- ja muut ohjeet.
...
toiminimi, mahdollinen aputoiminimi, yritys- ja yhteisötunnus, sen toimipaikan osoite, jossa toimintaa harjoitetaan, ja muut yhteystiedot;

vastuuhenkilön nimi, rekisteröintitunnus ja pätevyyttä koskevat tiedot;

liikkeen toiminnan ala;

rekisteröintitunnus;

toiminnanharjoittajalle ja vastuuhenkilölle tämän lain nojalla määrätyt seuraamukset.

Rekisteriin tallennetaan 14 ja 35 §:ssä tarkoitetuista pätevyytensä osoittaneista henkilöistä:

nimi ja henkilötunnus;

pätevyyttä koskevat tiedot;

rekisteröintitunnus;

pätevyystodistuksen peruuttaminen.
...
toiminnan ala, jota koe koski;

todistuksen antamisen päivämäärä.
...
tarkastuslaitokselle tämän lain nojalla määrätyt seuraamukset;

tarkastuslaitoksen alihankkijan nimi, toiminimi, mahdollinen aputoiminimi, yritys- ja yhteisötunnus, pätevyysalue, sen toimipaikan osoite, jossa toimintaa harjoitetaan, ja muut yhteystiedot.
...
pelastusviranomaiselle, kunnan rakennusvalvontaviranomaiselle sekä työsuojelu-, ympäristönsuojelu- ja kuluttajansuojaviranomaiselle, jos tiedon luovuttaminen on viranomaisen tehtävien suorittamisen kannalta välttämätöntä;
