...
joka on siittänyt lapsen;

jonka siittiöitä on käytetty äidin hedelmöittämiseen muulla kuin 1 kohdassa tarkoitetulla tavalla ja lapsi on syntynyt tämän tuloksena.
...
yhdessä pyytävät lastenvalvojalta vanhemmuuden selvittämistä ennen kuin kuusi kuukautta on kulunut lapsen syntymästä; tai

yhdessä hyväksyvät sen, että vanhemmuus selvitetään toisen henkilön tunnustettua tai ilmoitettua aikovansa tunnustaa lapsen 18 §:n 2 momentissa tarkoitetulla tavalla.
...
lapsesta ei ole saatu näytettä, josta oikeusgeneettinen isyystutkimus olisi voitu tehdä;
...
lastenvalvojan aloitteesta tai lapsen synnyttäneen äidin, 15 vuotta täyttäneen lapsen tai vanhempana itseään pitävän henkilön pyynnöstä, jos 2 momentin 1–3 kohdassa tarkoitetut keskeyttämisen edellytykset eivät enää täyty;

lapsen synnyttäneen äidin, 15 vuotta täyttäneen lapsen tai vanhemmuuden tunnustaneen henkilön pyynnöstä, jos 2 momentin 4 kohdassa tarkoitetut keskeyttämisen edellytykset eivät enää täyty.
...
lapsen synnyttänyt äiti, jollei hän ole 19 §:n tai 22 §:n 1 momentin mukaisesti hyväksynyt tunnustamista;

lapsen huoltaja, jos se on hankaluudetta järjestettävissä;

tunnustajan huoltaja tai muu laillinen edustaja, jos tunnustaja on alaikäinen.
...
jos lastenvalvoja on keskeyttänyt isyyden selvittämisen, koska lapsesta ei ole saatu näytettä, josta oikeusgeneettinen isyystutkimus olisi voitu tehdä; tai
...
lapsen synnyttänyt äiti ja aviomies tai vahvistettu vanhempi ovat asuneet erillään lapsen syntymän aikaan;

kantaja on asunut yhdessä lapsen synnyttäneen äidin kanssa lapsen syntymän aikaan ja osallistunut lapsen hoitoon tai jos kantajan ja lapsen välille on muutoin muodostunut perheyhteyteen rinnastettava suhde; ja
...
lapsen syntymän aikaan on tai lapsen syntymää edeltäneen vuoden aikana on ollut asuinpaikka Suomessa; tai

ei lapsen syntymän aikaan ole asuinpaikkaa missään valtiossa ja hän oleskelee tuolloin Suomessa tai on täällä turvapaikanhakijana.
...
sillä, jonka äitiydestä on kysymys, on lapsen syntymän aikaan asuinpaikka; tai

se, jonka äitiydestä on kysymys, lapsen syntymän aikaan oleskelee tai on turvapaikanhakijana, jos hänellä ei tuolloin ole asuinpaikkaa missään valtiossa.
...
lapsen synnyttäneellä lapsen syntymän aikaan on asuinpaikka Suomessa;

lapsen synnyttäneellä ei lapsen syntymän aikaan ole asuinpaikkaa missään valtiossa ja hän oleskelee tuolloin Suomessa tai on Suomessa turvapaikanhakijana.
...
lapsen synnyttäneellä lapsen syntymän aikaan on asuinpaikka; tai

lapsen synnyttänyt lapsen syntymän aikaan oleskelee tai on turvapaikanhakijana, jos hänellä ei tuolloin ole asuinpaikkaa missään valtiossa.
...
vastaajalla tai ainakin yhdellä vastaajista on tai viimeksi ennen kuolemaansa oli Suomessa asuinpaikka;
...
asiaa ei voida ratkaista siinä vieraassa valtiossa, jossa lapsella tai sillä, jonka vanhemmuudesta on kyse, on tai viimeksi ennen kuolemaansa oli asuinpaikka ja asian ratkaisemiselle Suomessa on erityinen syy.
...
on Suomessa asuinpaikka; taikka

ei ole asuinpaikkaa missään valtiossa, mutta hän oleskelee Suomessa tai on täällä turvapaikanhakijana.
...
päätöksen antaneen vieraan valtion viranomaisen toimivalta ei ole perustunut minkään osapuolen asuin- tai kotipaikkaan, kansalaisuuteen tai muuhun sellaiseen liittymään, joka huomioon ottaen viranomaisella olisi ollut perusteltu syy ottaa asia käsiteltäväkseen;

päätös on annettu pois jäänyttä vastaan eikä haastehakemusta tai vastaavaa asiakirjaa ole annettu tiedoksi poisjääneelle niin hyvissä ajoin ja sillä tavalla, että hän olisi voinut valmistautua vastaamaan asiassa;

päätös on ristiriidassa sellaisen Suomessa annetun vanhemmuutta koskevan päätöksen kanssa, jota koskeva oikeudenkäynti on tullut vireille ennen ulkomailla annettuun päätökseen johtanutta oikeudenkäyntiä;

päätös on ristiriidassa sellaisen vieraassa valtiossa aikaisemmin annetun vanhemmuutta koskevan päätöksen kanssa, joka tunnustetaan Suomessa;
...
on annettu valtiossa, jossa sillä, joka on vahvistettu äidiksi, on lapsen syntymän aikaan asuinpaikka ja jossa hän on asunut keskeytyksettä vähintään vuoden välittömästi ennen lapsen syntymää; tai

tunnustetaan siinä valtiossa, jossa sillä, joka on vahvistettu äidiksi, on lapsen syntymän aikaan asuinpaikka ja jossa hän on asunut keskeytyksettä vähintään vuoden välittömästi ennen lapsen syntymää.
...
tunnustajan yksilöinti- ja yhteystiedot, tiedon asiakirjasta, josta henkilöllisyys on todettu, kotikunnan ja tunnustamislausuman;

tunnustamisen hyväksyjän yksilöinti- ja yhteystiedot, tiedon asiakirjasta, josta henkilöllisyys on todettu, siviilisäädyn, kotikunnan, hyväksymislausuman, syntyvien lasten arvioidun lukumäärän ja lapsen arvioidun syntymäajan;

mahdollisesta hedelmöityshoitotodistuksesta ilmenevät tiedot;

tunnustamisen vastaanottajan nimen, tehtävänimikkeen, organisaation nimen ja vastaanottajan yhteystiedot;

muut kuin 1–4 kohdassa tarkoitetut tunnustamistapahtumaa tai sen osapuolia koskevat, 1 momentissa tarkoitettujen tehtävien suorittamiseksi välttämättömät, muut kuin erityisiin henkilötietoryhmiin kuuluvat tai viranomaisten toiminnan julkisuudesta annetun lain 24 §:n 1 momentin 32 kohdassa tarkoitetut salassa pidettävät tiedot.
