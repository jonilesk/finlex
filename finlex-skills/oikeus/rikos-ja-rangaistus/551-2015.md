...
rikoksesta epäillyn aikaisempi rikollisuus, rikoksesta epäillyn tekemän rikoksen tai tekemien rikosten laatu ja tuomitun rangaistuksen pituus;

rikoksesta epäillyn sosiaaliset olosuhteet, yksilölliset tarpeet ja voimavarat sekä toimintakyky;
...
seuraamuksen turvalliseen suorittamiseen vaikuttavat tekijät.
...
valvonnan järjestämisestä valvontatapaamisin, muin tuomitun ja valvojan välisin yhteydenpidoin, sähköisen valvonnan keinoin tai muilla valvontatavoilla;

Rikosseuraamuslaitoksen vastuuhenkilön ja rikoksesta epäillyn yhteystiedoista;

yhteydenpidon tiheydestä valvojan ja tuomitun välillä;

rikoksesta epäillyn asumista, työtä, koulutusta, opiskelua sekä sosiaalisen tilanteen ja muiden olosuhteiden edistämistä koskevista suunnitelmista;

työn, kuntoutuksen, koulutuksen tai muun toiminnan tavoitteista, sisällöstä, vaativuustasosta, suoritus- ja osallistumispaikoista sekä muista vastaavista seikoista;

tuomitulle kuuluvien velvollisuuksien rikkomisesta aiheutuvista seuraamuksista;

seuraamuksen suorittamisen päättymisen jälkeisistä toiminnan ja tukitoimenpiteiden jatkamisesta ja muista vastaavista seikoista;

tuomitulle annetuista muista määräyksistä.
...
asiakirjojen saapumisesta Rikosseuraamuslaitoksen yhdyskuntaseuraamustoimistolle yhdyskuntaseuraamuksen täytäntöönpanoa varten;

yhdyskuntaseuraamusten täytäntöönpanosta annetun lain 14 §:ssä tarkoitetun täsmennetyn rangaistusajan suunnitelman tiedoksiantopäivästä;

yhdyskuntaseuraamuksen suorittamisen aloittamispäivästä;

yhdyskuntaseuraamuksen suorittamisen määräaikaisesta lykkäyksestä tai keskeytyksestä;

yhdyskuntaseuraamuksen suorittamisen aloittamisen kieltämisestä ja keskeyttämisestä.
...
valvojan, apuvalvojan, tuomitun ja palveluntuottajan vastuuhenkilön yhteystiedot;

valvojan ja palveluntuottajan vastuuhenkilön välisen yhteydenpidon tarkoitus ja tapa;

järjestettävän hoidon tavoite, sisältö, aikataulu ja käytännön toteutustoimet;

hoitoon liittyvät tukitoimenpiteet;

tuomitun kirjallinen suostumus siihen, että päihdehuollon yksikössä suoritetun päihtymystilakokeen tulos voidaan luovuttaa täytäntöönpanosta vastaavalle Rikosseuraamuslaitoksen yhdyskuntaseuraamustoimistolle;

tuomitun laissa säädetyt ja lain nojalla määrätyt velvollisuudet sekä niiden noudattamisen valvonta ja epäillystä rikkomisesta ilmoittaminen.
...
järjestettävän hoidon tavoite, sisältö ja käytännön toteutus;

valvonta hoidon aikana;

hoitoon kuuluvat ohjelmat ja tehtävät;

hoidon aikataulu;

hoitoon liittyvät tukitoimenpiteet;

hoidon tarkoituksen toteutumisen seuranta;

tuomitun laissa säädetyt velvollisuudet sekä niiden noudattamisen valvonta, epäillyn rikkomisen selvittäminen ja kirjaaminen sekä rikkomisesta aiheutuvat seuraamukset hoidon kannalta;

tuomitun kirjallinen suostumus siihen, että päihdehuollon yksikössä suoritetun päihtymystilakokeen tulos voidaan luovuttaa täytäntöönpanosta vastaavalle Rikosseuraamuslaitoksen yhdyskuntaseuraamustoimistolle;

tuomitulle annetut määräykset.
...
järjestettävän hoidon tavoite, sisältö, aikataulu ja käytännön toteutus;
...
tarkastuksen aika ja paikka;

päätöksentekijä ja hänen virka-asemansa;

tarkastuksen suorittaja;

löydetyt esineet ja aineet; sekä
...
sisällöstä sekä edellyttämistä käynneistä sosiaali- ja terveydenhuollon toimintayksikössä;

erityisistä ehdoista ja niiden noudattamisen valvonnan sisällöstä ja menettelytavoista;

edellyttämästä yhteydenpidosta sosiaali- ja terveydenhuollon toimintayksikön, tuomitun kotikunnan, valvojan ja tuomitun välillä;

päättymisen tai keskeytymisen edellyttämistä toimenpiteistä;
...
viranomainen, jolle oikaisuvaatimus on toimitettava; sekä
...
todistus siitä, milloin päätös on annettu tiedoksi tai muu selvitys oikaisuvaatimusajan alkamisen ajankohdasta; sekä

asiakirjat, joihin tuomittu vetoaa oikaisuvaatimuksensa tueksi, jollei niitä ole aikaisemmin toimitettu viranomaiselle.
